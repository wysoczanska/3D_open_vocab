...
class VoteHeadWithFeatures(BaseModule):
    """VoteNet head with CLIP embedding RoI head."""
    def __init__(self,
                 num_classes,
                 bbox_coder,
                 train_cfg=None,
                 ...
                 iou_loss=None,
                 init_cfg=None):
        super(VoteHeadWithFeatures, self).__init__(init_cfg=init_cfg)
        ...
        if semantic_loss is not None:
            ...
            self.text_proj = nn.Parameter(torch.empty(768, 128)) # from CLIP
        ...
        self.emb_size = 128 # TODO: parametrize
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        # 3. predict bbox and score
        cls_out, reg_predictions = self.conv_pred(features)

        # logit_scale = self.logit_scale.exp()
        # vis_feat = cls_out[:, 2:, :] / (cls_out[:, 2:, :].norm(dim=1, keepdim=True) + 1e-6)
        # text_feat = class_features / (class_features.norm(dim=1, keepdim=True) + 1e-6)
        # S = (logit_scale * vis_feat) @ (text_feat.transpose(1, 2))

        # ugly workaround to pass
        # cls_pred (bs, objectness + clas preds, n)
        # cls_preds = torch.cat([cls_out[:, :2, :], S])

        # 4. decode predictions
        decode_res = self.bbox_coder.split_pred(cls_out,
                                                reg_predictions,
                                                aggregated_points)
    ...
    def loss(self,
        ...
        targets = self.get_targets(points, gt_bboxes_3d, gt_labels_3d,
                                   pts_semantic_mask, pts_instance_mask, bbox_preds, text_features
                                   )
        (vote_targets, vote_target_masks, size_class_targets, size_res_targets,
         ...
         valid_gt_weights, mask_features) = targets
    ...
    def get_targets(self,
        ...
        (vote_targets, vote_target_masks, size_class_targets, size_res_targets,
         ...
         objectness_masks, mask_features) = multi_apply(self.get_targets_single, points,
                                         gt_bboxes_3d, gt_labels_3d,
                                         pts_semantic_mask, pts_instance_mask,
                                         aggregated_points, text_features)
        ...
        return (vote_targets, vote_target_masks, size_class_targets,
                ...
                box_loss_weights, valid_gt_weights, mask_features)
    ...
    def get_targets_single(self,
        ...
        if len(text_features) == 0:
            text_features = torch.zeros(1, text_features.shape[1]).to(text_features.device)

        mask_targets = gt_labels_3d[assignment]
        mask_features = text_features[assignment]
        ...
        return (vote_targets, vote_target_masks, size_class_targets,
                ...
                mask_targets.long(), objectness_targets, objectness_masks, mask_features)
